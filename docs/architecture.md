...
`directory-drift-checker` uses a small `src/` package with two layers:
...
The source reference `pnp/cli-microsoft365` influenced only the problem-space research. Implementation details are intentionally independent.
